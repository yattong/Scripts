...
# Date : 5 May 2018
# Version : 1.0
# Purpose : Failover NSX load Balancers
# Parameters : Edge Name, Username, Password
# Assumption : 
#
##################################################################################

######### SRM Passed Variables
# $env:VMware_RecoveryName
# $env:VMware_RecoveryMode
# $env:VMware_VC_Host

############ Set Up logging ################
$logdir = "C:\SRM-Logs\$($env:VMware_RecoveryName.substring(0,6))"
...
$logfile = "$($logdir)\$($env:VMware_RecoveryName.substring(0,6))_NSX_LB_$($env:VMware_RecoveryMode.ToUpper()).log"

############# Start Logging ################
"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : --------------------- $($env:VMware_RecoveryName.substring(0,6)) DR INVOKED -------------------------" >> $logfile


########### Log out Variables ##############
"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : SRM Variable VMware_RecoveryName = $($env:VMware_RecoveryName)" >> $logfile
"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : SRM Variable VMware_RecoveryMode = $($env:VMware_RecoveryMode)" >> $logfile
"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : SRM Variable VMware_VC_Host = $($env:VMware_VC_Host)" >> $logfile
#"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : $(Get-ChildItem env:*)" >> $logfile

########### Derived Variables  #############
# EdgeName will enable search for all LB's for application id
$edgeName = "VEDG$($env:VMware_RecoveryName.substring(0,6))"

## try catch
try {
# Determine Recovery Site
if ($env:VMware_VC_Host -eq "prdvvcsha04.kpmgmgmt.com"){
...
if (Get-Module -Name VMware.VimAutomation.Core){
    "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : VMware PS Modules loaded successfully" >> $logfile  
} else {
    "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") ERROR : VMware PS Modules loading FAILED" >> $logfile  
}
...
######################### DR Recovery Execution #############################

####### Test if Hole in the Ground or planned migration  ###########
if ($env:VMware_RecoveryMode -eq "recovery"){
    if ((Test-NetConnection $protectedVC -Port 443).TcpTestSucceeded){
        $recoveryDR = $true
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Able to contact $($protectedVC), assuming Planned Migration Scenario" >> $logfile
    } else {
        $recoveryDR = $false
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Unable to contact $($protectedVC), assuming real DR Scenario" >> $logfile
    }
} 


############## If planned migration or Test Recovery  ##############
if ($recoveryDR -or $env:VMware_RecoveryMode -eq "test"){

    "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : This is an SRM TEST DR of PLANNED MIGRATION11 Scenario" >> $logfile

    # if LPR -> IXE shutdown LB VMs
    if($protectedVC -eq "prdvvcsha03.kpmgmgmt.com"){
        # Connect to vCenter
    ...
    # if IXE to LPR - Disconnect the LB NIC
    if ($protectedVC -eq "prdvvcsha04.kpmgmgmt.com"){
        ## Connect to NSX
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Pulling List of all Edge Devices from $($protectedNSX)..." >> $logfile
        ...
        $nsxEdgeIds = @();
        "Searching for All APPID Edge Device ID beginning with Edge Name $($edgeName)..." >> $logfile
        ...
        foreach ($nsxEdgeId in $nsxEdgeIds){
            #$nsxEdgeId
            "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Pulling Edge Device Config of $($nsxEdgeId)..." >> $logfile
            ...
            "Updating Edge Device $($nsxEdgeId) with NIC Disconnected..." >> $logfile
...
if ($protectedVC -eq "prdvvcsha03.kpmgmgmt.com"){
    ## Connect to NSX
    "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Pulling List of all Edge Devices from $($recoveryNSX)..." >> $logfile
    ...
    foreach ($nsxEdgeId in $nsxEdgeIds){
        #$nsxEdgeId
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Pulling Edge Device $($nsxEdgeId) Config..." >> $logfile
        ...
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Updating Edge Device $($nsxEdgeId) with NIC Connected..." >> $logfile
...
if ($protectedVC -eq "prdvvcsha04.kpmgmgmt.com"){
    ## Power On LB VMs
    # Connect to vCenter
    "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Connecting to vCenter : $($env:VMware_VC_Host)" >> $logfile
    $session = Connect-VIServer $env:VMware_VC_Host -User svc_sha_srm@kpmgmgmt.com -Password $vcPassword -ErrorAction Stop

    # logging
    if ($session){
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Connected to vCenter : $($env:VMware_VC_Host)" >> $logfile
        ...
        # Disconnect VC
        Disconnect-VIServer * -Force -Confirm:$false
        "$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : Disconnected from vCenter : $($env:VMware_VC_Host)" >> $logfile
    }
}


############# Success Logging ################
"$(Get-Date -Format "[yyyy-MM-dd]HH:mm:ss -") INFO : --------------------- $($env:VMware_RecoveryName.substring(0,6)) DR INVOCATION SUCCESSFUL -------------------------" >> $logfile
